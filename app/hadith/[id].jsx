...
import { logHadithRead } from '../../src/services/hadithLogs';
...
export default function HadithDetail() {
    ...
    const loadHadith = async () => {
        try {
            const { data, error } = await supabase
                .from('hadiths')
                .select(
                    'id, arabic, full_text, book, hadith_number, grade, reference'
                )
                .eq('id', id)
                .single();
            ...
            // Log this Hadith as read for today's Hadith streak.
            // Logging failure must never prevent the Hadith from being displayed.
            try {
                await logHadithRead(data.id);
            } catch (logError) {
                console.warn(
                    'Hadith log failed:',
                    logError.message
    ...
    const handleShare = async () => {
        if (!shareCardRef.current || sharing) {
            return;
        }
